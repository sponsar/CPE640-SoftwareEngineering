...
public class BallWorld extends JPanel {
    ...
    */
   public BallWorld(int width, int height) {//640,480
...
//        balls[4] = new Ball(550, 300, 25, -3, 0, Color.MAGENTA);
      ...
      this.addComponentListener(new ComponentAdapter() {//这是重新调整canvas和ballworld的大小，把canvas填满ballworld
      ...
      Thread gameThread = new Thread() {
         public void run() {
            while (true) {
               ...
               /*try {
                  Thread.sleep(timeLeftMillis);//一秒多少帧
   ...
   public void gameUpdate() {
      ...
      do {
         ...
         float tMin = timeLeft;//tMin系下一次碰撞嘅时间
         ...
         for (int i = 0; i < currentNumBalls; i++) {
            balls[i].update(tMin);//将每个波移到下一个位置，帧与帧之间的距离
         }
   ...
   class DrawCanvas extends JPanel {
      ...
      public void paintComponent(Graphics g) {
         ...
         g.setColor(Color.WHITE);//这些是字体颜色大小的设定
      ...
      @Override
      public Dimension getPreferredSize() {//无咩用
         ...
         speedControl.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
               ...
               if (!source.getValueIsAdjusting()) {
                  ...
                  for (int i = 0; i < currentNumBalls; i++) {
                     balls[i].speedX = ballSavedSpeedXs[i] * percentage / 100.0f;//速度系哩度改变
